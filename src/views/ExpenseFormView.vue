...
// Determine if we're editing an existing expense or creating a new one
const isEditMode = computed(() => route.name === 'expense-edit');
...
const validateForm = () => {
  ...
  if (!expense.value.amount) {
    ...
    isValid = false;
  } else if (isNaN(expense.value.amount) || parseFloat(expense.value.amount) < 0) {
    errors.value.amount = 'Kwota musi być liczbą dodatnią';
    isValid = false;
  }

  if (!expense.value.date) {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      await expenseStore.updateExpense(expenseId.value, formData);
      alert('Wydatek zaktualizowany pomyślnie');
    } else {
      await expenseStore.createExpense(formData);
      alert('Wydatek utworzony pomyślnie');
    }

    // Navigate back to expense list
    router.push({ name: 'expenses' });
  } catch (error) {
    alert(`Błąd ${isEditMode.value ? 'aktualizacji' : 'tworzenia'} wydatku: ${error.message || 'Nieznany błąd'}`);
  }
};
...
const cancel = () => {
  if (isEditMode.value) {
    router.push({ name: 'expense-details', params: { id: expenseId.value } });
  } else {
    router.push({ name: 'expenses' });
  }
};
  ...
  if (isEditMode.value) {
    try {
      ...
      if (expenseStore.expense) {
        // Copy expense data to form
        expense.value = {
          name: expenseStore.expense.name,
          amount: expenseStore.expense.amount,
          category_id: expenseStore.expense.category_id || null,
          date: expenseStore.expense.date
        };
      }
    } catch (error) {
...
<template>
  <div class="expense-form">
    <h1>{{ isEditMode ? 'Edytuj wydatek' : 'Utwórz nowy wydatek' }}</h1>

    <div v-if="isEditMode && expenseStore.loading" class="loading">
      Ładowanie danych wydatku...
    </div>

    <div v-else-if="isEditMode && expenseStore.error" class="error">
      {{ expenseStore.error }}
    </div>
